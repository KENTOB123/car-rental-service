...
const ParamsSerch = ({ car, id }: {car: CAR_DATA, id: number }) => {
  const searchParams = useSearchParams();
    const queryString = searchParams.toString();
    const jsonQuery = queryString.split('=')[1]; 
    const jsonQueryCar = JSON.parse(decodeURIComponent(jsonQuery)); 
     
    console.log(jsonQueryCar.name);
    //APIでまとめた方がいいかも？ searchParamsから必要な情報を1つずつ抜き出してSerchに返すのもアリ。
    ...
    <div className='flex flex-col mt-20 items-center'>
        ...
        <div className='flex w-2/3 justify-around'>
          ...
          <div className='flex flex-col w-1/4 gap-2'>
            ...
            <VehicleModal 
                carType={jsonQueryCar.name}
                pickUp={selectedUp}
                dropOff={selectedOff}
                upDay={pickupDate}
                offDay={returnDate}
                selectedCar={jsonQueryCar}
